...
长时间运行的任务可能持续产生 stdout/stderr，不能等到 `reply.task_result` 才一次性展示。Pulse 必须把运行中输出视为 task state 的一部分，继续通过 heartbeat / group heartbeat 的 `PulseMessage` 链路传输，不新增任何 API。

### 设计目标

- 支持任意文本或字节输出，不要求任务输出必须是 JSON。
- 支持长任务运行中持续展示最近输出，避免 UI 长时间空白。
- 保留当前 completion queue 语义：最终结果仍由 `reply.task_result` 或 `reply.task_result_chunk` 入 completion queue。
- 不改变 agent 每轮只写一个 coordinator 的约束；follower 输出先到 leader，再由 leader 的 group heartbeat 聚合上报。
- 不新增 task streaming API、WebSocket、SSE 或额外端口；已有 `/api/agents/{agent}/tasks` snapshot 可以携带只读 stream 视图。
...
- agent 将输出按大小或时间切片形成 `reply.task_output_append`：
  ...
  - flush 间隔建议不超过 `1s` 或每轮 heartbeat 至少 drain 一次。
...
- 任务结束时仍发送 `reply.task_result`：
  ...
  - 如果最终结果太大或不是 JSON，completion viewer 必须至少展示 tail、大小、编码和可拷贝内容。

### Group Leader 行为

- follower 发送到 leader 的 `/group/heartbeat` 中可以包含 `reply.task_output_append`。
- leader 不解析 payload 内容，只做消息转发和按 agent 聚合。
- leader 可按 group heartbeat 请求大小做安全裁剪：
  - 优先保留每个 running task 的最新 chunk。
  - 超过单次 batch 上限时设置 `tail_truncated=true`，让 coordinator/UI 知道输出窗口发生过丢弃。
- leader 不能因为某个 follower 输出量大而阻塞整个 group heartbeat。
...
- coordinator 为每个 running task 维护有限 stream buffer：
  ...
  - 记录 `first_stream_seq`、`last_stream_seq`、`stream_bytes`、`dropped_bytes`、`tail_truncated`。
- coordinator 不把 stream chunk 放入 completion queue；completion queue 只保存最终 `TaskResult`。
- coordinator snapshot 可以通过已有 `/api/agents/{agent}/tasks` 返回 `output_streams` 或 `stream_tail` 字段，供 Run UI 展示运行中输出；这属于既有 task snapshot 扩展，不是新增 API。
- coordinator peer sync 仍只通过已有 state lazy sync 机制传播必要 task state；不要求 stream tail 在 peers 间强一致。

### UI 行为

- Run UI 在任务 `accepted/running` 且 completion 未返回时，优先展示 stream tail。
- stream viewer 必须支持：
  - 原始文本展示。
  - 自动滚动到最新输出，并允许用户暂停自动滚动。
  - 拷贝当前 tail。
  - 明确显示 `stdout/stderr`、输出大小、是否 `tail_truncated`。
- 如果 stream 内容是 JSONL，可以按行做轻量高亮；普通文本不强制 JSON 格式化。
- completion 到达后，右侧结果区切到 completion viewer；stream tail 可作为 trace/运行中详情保留最近摘要。

### 背压与安全边界

- 单 task stream buffer 必须有内存上限，禁止无界保存 stdout/stderr。
- 单 heartbeat 或 group heartbeat 的 stream chunk 数必须有上限，避免输出型任务压垮心跳链路。
- 当输出生产速度超过心跳传输能力时，系统选择“保留最近 tail、丢弃旧 chunk”，而不是阻塞 heartbeat。
- 被丢弃的字节数和 chunk 数必须可见，至少通过 `tail_truncated` 和 `dropped_bytes` 暴露给 UI。
