...
- 为 group leader 增加可落库的时序统计口径，支持后续通过 coordinator 本地 DB 分析 group 心跳上报方案的降压收益、覆盖率、稳定性和 fallback 成本。
...
- group leader 每轮批量上报必须保留可统计字段，避免只能从当前态推断方案效果。
...
group 心跳方案上线后，需要通过 coordinator 本地 DB 评估以下问题：

- 请求降压：每个 heartbeat interval 的 coordinator 实际请求数是否接近 `group_count`，而不是 `agent_count`。
- 成员覆盖：每个 group 的 `submitted_agent_count / member_count` 是否长期接近 `1`。
- 接收质量：`accepted_agent_count` 是否接近 `submitted_agent_count`，`rejected_agent_count` 是否接近 `0`。
- leader 稳定性：同一个 `group_id` 在观察窗口内的 `leader_agent_id` 变化次数是否可控。
- stale plan：leader 切换或 group recompute 后，`stale_member_count` 是否出现持续尖峰。
- fallback 成本：leader 不可用时 `direct_fallback_count` 是否导致 coordinator 请求数回升。
- 额外延迟：`leader_collect_ms` 与 `group_latency_ms` 是否抵消批量上报带来的收益。

这些指标对应的本地时序表见 `docs/design/coordinator-local-storage-timeseries.md` 的 `group_leader_sample`。
...
- 在可用时附带 `collector_sent_at_ms`、`leader_collected_at_ms`、成员收集结果摘要，供 coordinator 写入 group leader 时序统计。
...
- `expired` host 不参与新 group，但在 `PULSE_EXPIRED_HOST_RETENTION_MS`
  观察窗口内仍保留在 host 视图中；默认 `300000ms`。窗口结束后 coordinator
  必须清理该节点的当前态、group plan 和内存任务状态。后续心跳可将节点作为
  新节点重新加入。
...
- batch size
- group leader coverage ratio
- stale member rejected count
- direct fallback count
- leader collect latency
- group heartbeat arrival gap
- coordinator 本地 DB 写入 `group_leader_sample`，用于离线 SQL 分析和后续评估报告。
...
group leader 时序统计不改变控制面事实源，只作为方案评估与问题排查的数据面：

- 当前态仍由 coordinator 内存中的 `states`、`groupPlans`、`groupViews` 决定。
- 本地 DB 只保存观测样本，不参与 leader 选择、plan 下发或任务执行判定。
- 写入失败不能影响 heartbeat 快路径；最多降低历史分析完整性。
- 多 coordinator 场景下，每个 coordinator 只保存本地观察到的 group leader 样本，不要求强一致。

每次 coordinator 处理带 `group_id + agents[]` 的 group heartbeat 时，生成一条 `group_leader_sample`：

| 字段 | 来源 | 作用 |
| --- | --- | --- |
| `group_id` | 请求字段 | group 维度主键 |
| `leader_agent_id` | 请求来源或 group plan | 统计 leader 切换 |
| `member_count` | 当前 `GroupView.members` | 期望覆盖规模 |
| `submitted_agent_count` | 请求 `agents[]` 数量 | 实际批量覆盖 |
| `accepted_agent_count` | 每个 agent 的 seq 接收结果 | 接收质量 |
| `rejected_agent_count` | 非成员、旧 seq 或协议拒绝 | stale plan 与异常识别 |
| `missing_member_count` | `member_count - submitted_agent_count` | follower 缺口 |
| `direct_fallback_count` | 当前态或事件近似统计 | fallback 成本 |
| `leader_collect_ms` | leader 可选上报时间差 | follower 到 leader 汇聚耗时 |
| `group_latency_ms` | coordinator 观测时间与 leader 发送时间差 | leader 到 coordinator 链路耗时 |
| `arrival_gap_ms` | 同 group 上次 observed 时间差 | group heartbeat 抖动 |

事件侧需要补充：

- `group.leader_changed`：同一 `group_id` 的 leader 发生变化。
- `group.heartbeat_partial`：本轮 `submitted_agent_count < member_count`。
- `group.stale_member_rejected`：leader 提交了不属于当前 group 的 member。
- `group.direct_fallback`：follower 因 leader 不可用临时 direct 上报。

典型 SQL 分析：

- 按 cluster/area/group 计算 group 覆盖率、拒绝数和 fallback 数。
- 按 group 统计 leader 切换次数、最大 `arrival_gap_ms` 和 stale member 尖峰。
- 对比 `agent_count`、`group_count`、`coordinator_requests`，评估真实降压比例。
