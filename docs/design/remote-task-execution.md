...
- 当前只允许预定义任务；默认参数必须是 `--dry-run`，非 dry-run 只能通过隐藏参数入口显式输入。
- Run UI 支持单 host 执行，也支持按集群向当前集群内所有 host 批量下发作业。

预定义任务 allowlist：

| Task ID | Script | 默认参数 | 说明 |
| --- | --- | --- | --- |
...
- task 默认参数必须是 `--dry-run`，Run UI 默认路径不能产生非 dry-run 操作。
- 自定义参数不是公开主功能，只作为受控运维入口隐藏在 Run UI 中，避免误触。
- 隐藏入口不是安全边界；真正的安全边界仍是 task allowlist、参数校验、审计 trace 和后续权限系统。
...
- 每个 cluster header 增加 `Run UI` 按钮。
- 点击后弹出任务框。
- 任务框展示该 agent 的 execution queue。
- 任务框提供预定义 task 选择和 run 操作；任务选择展示业务名，不展示 `dry-run` 标记，避免把默认参数误当作任务身份。
- 默认执行参数为 `--dry-run`。
- 5 秒内连续 3 次点击 `结果查看` 标题后，才显示自定义参数输入框。
- 参数输入框出现后，允许运维人员输入自定义参数；空参数仍回退到 `--dry-run`。
- 非 dry-run 操作会真实修改线上机器，UI 必须在参数区域提示风险。
- 集群 Run UI 使用同一个任务框，目标区显示集群名和 host 数；执行时对集群内 host 逐台调用现有 per-agent task enqueue API。
...
- 安全默认路径：面向日常巡检、预检查和观测，默认必须是 dry-run 或只读参数。
- 破坏性路径：面向修复、变更和批量操作，必须刻意触发，并且保留审计线索。

本阶段选择“隐藏参数入口 + allowlist task + 默认 dry-run”的组合，而不是直接暴露完整参数表：

- 优点是主路径简单，不增加普通巡检误触概率。
- 优点是复用现有 heartbeat control plane，不新增 agent API，也不要求 agent 开端口。
- 优点是自定义参数仍由 coordinator 进入 `cmd.task_execute`，可以在 trace 中记录。
- 风险是隐藏点击不是鉴权，不能替代用户身份、审批、二次确认和变更窗口。
- 风险是集群批量执行会放大错误参数影响，因此第一版仍默认 `--dry-run`，后续必须接入 RBAC、审计和二次确认。

集群执行第一版不新增 `/api/clusters/{cluster}/tasks`：

- UI 根据 `/api/hosts` 的当前 cluster 分组得到 host 列表。
- 点击 cluster `Run UI` 后，任务框进入 cluster scope。
- 执行时 UI 对每个 host 调用 `/api/agents/{agent_id}/tasks`。
- coordinator 仍只维护 per-agent execution queue，避免引入新的 cluster queue 权威。
- 部分下发失败时，UI 展示失败数量；已成功入队的 host 不回滚。
- 未来如果需要强一致批量语义，再引入 coordinator-side cluster task batch、批次 ID、失败重试和批量审计视图。

参数输入规则：

- 请求体可以携带 `args: list<string>`。
- 未携带 `args`、空数组或空字符串时，coordinator 回退到任务默认参数 `["--dry-run"]`。
- coordinator 必须限制参数数量和单参数长度，禁止换行、NUL 等不适合作为进程参数的字符。
- agent 使用 `ProcessBuilder` 参数数组执行脚本，不能拼接 shell 字符串，避免 shell 注入。
- 自定义参数必须进入 trace detail，方便后续审计。
...
RemoteTask {
  ...
  script_path: string
  args: list<string>             // default ["--dry-run"], custom args are explicit operator input
