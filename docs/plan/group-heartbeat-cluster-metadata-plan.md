...
- `HostTilesPage`：
  ...
  - `弹出结果` 改回真正 queue pop 语义：调用 `/pop`，服务端只移除 completion queue head，刷新后展示下一个 item；保留 `/keep` 只作为只读保留动作，不再绑定弹出按钮。
  - Trace 区固定展示最近 4 条，防止重复结果操作把 Run UI 左侧撑满。
...
- 更新 `AgentTaskRunnerTest`：
  - 长任务 stream output 测试迁移到 `docs/plan/task-output-streaming-plan.md`。
- 更新 `GroupHeartbeatCollectorTest`：
  - group stream 聚合状态机测试迁移到 `docs/plan/task-output-streaming-plan.md`。
- 更新 `CoordinatorHttpServerTest`：
  ...
  - task snapshot stream tail 与 Run UI stream viewer 测试迁移到 `docs/plan/task-output-streaming-plan.md`。
