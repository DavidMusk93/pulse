...
def log_timestamp():
    return dt.datetime.now(dt.timezone.utc).astimezone().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
...
def load_all_blocks(db_paths, tide_home, now_epoch, trace):
    ...
    for db_path in sorted(valid_db_paths):
        try:
            ...
            raise RuntimeError('{} failed to load: {}'.format(db_path, exc)) from exc
    ...
    with ThreadPoolExecutor(max_workers=len(disk_paths)) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                raise RuntimeError('{} failed to scan untracked blocks: {}'.format(disk_path, exc)) from exc
    ...
    with ThreadPoolExecutor(max_workers=worker_count) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                raise RuntimeError('{} failed to remove block files: {}'.format(resource_key, exc)) from exc
...
def delete_candidates(db_delete_plan, trace):
    ...
    for (db_path, paths) in sorted(paths_by_db.items()):
        try:
            ...
            raise RuntimeError('{} failed to cleanup: {}'.format(db_path, exc)) from exc
    deleted = 0
...
def main(argv):
    ...
    report_time = dt.datetime.now(dt.timezone.utc).astimezone().strftime('%Y-%m-%d %H:%M:%S %Z')
    if plan.dry_run:
